    ...
    if (
      ...
      if (member) {
        ...
        let messageReason = msgArgs[2]
          ? message.content.substring(msgArgs.slice(0, 2).join(" ").length + 1)
          : "No reason provided";
        try {
          member.ban({ reason: messageReason });
          message.channel.send(`${member.user.tag} was banned for the reason: ${messageReason}`);
        } catch (error) {
          //Error handler
